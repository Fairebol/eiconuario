<div class="flex flex-col h-full w-full font-bold text-white background select-none">
    ...
    <div class="flex w-full h-[10vh] absolute z-20 text-white">
        <a href="<?= LOCALHOST ?>/course/<?= $page - 1?>" class="p-6 lg:p-4 border-black fixed bottom-0 lg:bottom-auto lg:top-0 left-0 align-middle bg-emerald-500 rounded-lg hover:bg-emerald-600 transition-transform duration-150 transform active:scale-90"><i class="fas fa-step-backward"></i> Anterior    </a>
        <a href="<?= LOCALHOST ?>/course/<?= $page + 1?>" class="p-6 lg:p-4 border-black fixed bottom-0 lg:bottom-auto lg:top-0 right-0 align-middle bg-emerald-500 rounded-lg hover:bg-emerald-600 transition-transform duration-150 transform active:scale-90">Siguiente <i class="fas fa-step-forward"></i></a>
    </div>
    ...
    <div class="grid h-full w-full justify-center items-center align-top grid-cols-3 md:grid-cols-4 lg:grid-cols-5 my-8 space-y-2 md:space-y-4">
        ...
        <div class="justify-center relative transform hover:scale-110 z-10 flex place-items-center place-content-center">
            <?php if ($u['role_id'] == 2) { ?>
            <img src="../../public/img/<?=$u['pathimg']?>" class="w-1/2 border-8 border-b-[25px] border-yellow-300" alt="">
            <?php } else {?>
